...
from models.net_relu_flatten import MobileNetV1 as MobileNetV1
from models.net_relu_flatten import FPN as FPN
from models.net_relu_flatten import SSH as SSH
import numpy as np
import pdb
from at5k_1.qconv2d import wr_str
...
class RetinaFace(nn.Module):
    ...
    def forward(self, inputs):
        f = open('E:/AT5000/AT5K_Pytorch_2/Pytorch_Retinaface_master/at5k_1/model_txt/model.txt', 'w')
        f.write('//layer0')
        f.write('\n')
        f.close()
        v = []
        ...
        # [1,3,375,500]
        v.append(self.dequant(inputs))
        out1 = self.body(inputs) #mobile_net
        fr = open("./bias","a")
        fr.write(str(out1[0]))
        fr.close()
        # v.append(self.dequant(out1[0]))
        # v.append(self.dequant(out1[1]))
        # v.append(self.dequant(out1[2]))
        fpn = self.fpn(out1)
        # v.append(self.dequant(fpn[0]))
        # v.append(self.dequant(fpn[1]))
        # v.append(self.dequant(fpn[2]))
        # out1 = fpn
        # list len = 3
        ...
        # v.append(self.dequant(feature1))
        # feature1.shape = [1,64,34,60]
        #wr_str('//ssh2')
        feature2 = self.ssh2(fpn[1])
        # v.append(self.dequant(feature2))
        # feature2.shape = [1,64,17,30]
        #wr_str('//ssh3')
        feature3 = self.ssh3(fpn[2])
        # v.append(self.dequant(feature3))
        # feature3.shape = [1,64, 9,15]
        features = [feature1, feature2, feature3]
        #bbox_regressions = torch.cat([self.BboxHead[i](feature) for i, feature in enumerate(features)], dim=1)
        bbox_regressions0 = self.BboxHead[0](features[0]) # [ 1, 4080, 2]
        # v.append(self.dequant(bbox_regressions0))
        bbox_regressions1 = self.BboxHead[1](features[1]) # [ 1, 1020, 2]
        # v.append(self.dequant(bbox_regressions1))
        bbox_regressions2 = self.BboxHead[2](features[2]) # [ 1,  270, 2]
        # v.append(self.dequant(bbox_regressions2))

        conf0 = self.ClassHead[0](features[0])
        # v.append(self.dequant(conf0))
        conf1 = self.ClassHead[1](features[1])
        # v.append(self.dequant(conf1))
        conf2 = self.ClassHead[2](features[2])
        # v.append(self.dequant(conf2))

        # ldm_regression0 = self.LandmarkHead[0](features[0])
        # ldm_regression1 = self.LandmarkHead[1](features[1])
        # ldm_regression2 = self.LandmarkHead[2](features[2])
        try:
        ...
        # ldm_regression0 = self.dequant(ldm_regression0)
        # ldm_regression1 = self.dequant(ldm_regression1)
        # ldm_regression2 = self.dequant(ldm_regression2)
        ...
        # ldm_regressions = torch.cat([ldm_regression0, ldm_regression1, ldm_regression2], dim=1)
